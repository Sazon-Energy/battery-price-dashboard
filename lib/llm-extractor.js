...
// No `minimum`/`maximum` on confidence_score - structured outputs doesn't
// support numeric constraints. Clamped in code after the response comes back.
const EXTRACTION_SCHEMA = {
  ...
  properties: {
    ...
    confidence_score: {
      type: 'number',
      description: 'Confidence in this extraction overall, from 0 (guessing) to 100 (certain).'
    }
  },
  required: ['is_battery', 'name', 'price', 'capacity_kwh', 'power_w', 'confidence_score'],
 ...
 * @returns {Promise<{ is_battery: boolean, name: string, price: number|null, capacity_kwh: number|null, power_w: number|null, confidence_score: number }|null>}
 */
export async function extractBatteryInfoWithLLM({ pageText, url }) {
  ...
  try {
    ...
    return {
      ...result,
      confidence_score: Math.max(0, Math.min(100, result.confidence_score))
    };
  } catch (error) {
